...
class GaussianDiffusion(nn.Cell):
    ...
    def ddim_sample(self, shape, clip_denoised=True):
        ...
        times = np.linspace(-1, total_timesteps - 1, sampling_timesteps + 1).astype(
            np.int32)  # [-1, 0, 1, 2, ..., T-1] when sampling_timesteps == total_timesteps
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            x_start = Tensor(x_start) if x_start is not None else x_start
            self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(Tensor(img, mindspore.float32), Tensor(time_cond),
                                                             self_cond, clip_denoised)
